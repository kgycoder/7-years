...
function _renderLyrics() {
    ...
    LY.lines.forEach((line, i) => {
        ...
        el.textContent = line.text;
        el.addEventListener('click', e => seekToLyric(line.start + 0.05, e));
        // Apple Music 스타일: Stagger 애니메이션으로 부드럽게 진입
        el.style.animation = `appleSpringSlideIn 0.42s var(--spring) forwards`;
...
/* ════════════════════════════════════════════
   [핵심] _highlightLine — Apple Music 스타일 Spring 애니메이션
════════════════════════════════════════════ */
function _highlightLine(idx, instant) {
    ...
    LY.lines.forEach((line, i) => {
        if (!line.el) return;
        const wasActive = line.el.classList.contains('active');
        ...
        if (d === 0) {
            ...
            // Spring animation 재트리거: 강제 reflow로 animation 다시 시작
            if (wasActive) {
                line.el.style.animation = 'none';
                void line.el.offsetHeight; // Force reflow
                line.el.style.animation = '';
            }
        } else if (d === -1) {
            line.el.classList.add('prev');
        } else if (d >= 1 && d <= 3) {
            line.el.classList.add('near');
        }
    });
    ...
    if (idx >= 0 && LY.lines[idx]?.el) {
        ...
        if (!scroll) return;
        const target = el.offsetTop - scroll.clientHeight / 2 + el.offsetHeight / 2;
        // Apple Music 스타일: smooth scroll + Spring transition 동시 적용
        scroll.scrollTo({ top: target, behavior: 'smooth' });
    }
}
